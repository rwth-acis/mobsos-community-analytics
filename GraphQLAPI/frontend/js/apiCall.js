...
Y({
  ...
  y.share.map.observe(function (event) {
    if (event.type === 'update' && document.getElementById('collabToggle').checked == true) {
      ...
      if (
        document.getElementById('SecondRequest').value == '' &&
  ...
  document.getElementById('queryForm').onsubmit = function (event) {
    if (document.getElementById('collabToggle').checked == true) {
      ...
      if (document.getElementById('SecondRequest').style.display != 'none') {
        y.share.map.set(
          'requestSecond',
          JSON.stringify(document.getElementById('SecondRequest').value)
        );
      }
    }
  };
...
function executeQuery(query, visual, outputID) {
  if (document.getElementById('DatabaseSelect').value == 'All') {
    ...
    var requestType = document.getElementById('RequestType').value;
    //var path = "http://localhost:9000/GraphqlAPITest/graphql/graphqlrest/graphql?query=";
...
function createGeoChart(data, outputID) {
  var output =
    '{"bw_author":[{"authorname":"Germany", "id":"49"}, ' +
  ...
  // Set chart options
  var options = { title: 'Query Visualization', width: 400, height: 300 };
...
function createLineChart(data, ouputID) {
  var output =
    '{"bw_author":[{"authorname":"Test 0", "id":"49", "id2":"19"}, ' +
...
function createBaseChart(data, type, outputID) {
  var output =
    '{"bw_author":[{"authorname":"Test 0", "id":"49"}, ' +
...
function createCalendarChart(data, outputID) {
  ...
  // Set chart options
  var options = { title: 'Query Visualization', width: 1000, height: 1000 };
